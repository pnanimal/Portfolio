...
<h3>RNN</h3>
<p> Similar to the ANN, the RNN needed to have input modified from the CNN 4D tensor. This is because the RNN processes data in a sequence like speech or time sequences. For this reason, the RNN could not understand all of the different kinds of inputs, and performed well by using the axis that contained the MFCC’s (Figure 11) only. The RNN takes the MFCC input into a Long Short-Term Memory (LSTM) layer (Bengio, et. al., 1994) which carries information along a sequence, which in this case corresponds to time. This layer contains 128 hidden layers with relu activation functions (Figure 16). There are four dense layers, and three dropout layers to prevent overfitting the training data. The final layer outputs 24 softmax outputs, corresponding to the 24 species. The Adam optimizer is used over 50 epochs with a loss function to maximize accuracy.  The RNN achieved 96.4% accuracy on the training data and 93.4% accuracy on the test data.</p>
<p><center>
